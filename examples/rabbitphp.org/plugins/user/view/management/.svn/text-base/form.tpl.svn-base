...
<form method='POST' action='/user/management/save'>
  <div id='user-registration-form'>
	  ...
	  <div class="floatleft">
	  	<div class="formfield roles">
	  	  <label for='user.roles'>Roles:</label>
	  		{validation_messages model='user.roles'}
	  	  <div class='field'>{input type='select' options=$roles option_name='name' option_value='id' name='roles[]' multiple='true' size='5'} <a href='javascript: void(0);' onclick='show_role_form(); return false;'>add role</a></div>
	  	</div>
	  </div>
